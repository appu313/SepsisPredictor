...
def grid_search_tune_parallel(
    ...
    if not TRAINING_DIR.is_dir():
        raise AttributeError(
            "Could not locate training sets within data fold directory"
        )
    elif not VALIDATE_DIR.is_dir():
        raise AttributeError(
            "Could not loacate validation sets within data fold directory"
        )
    train_inner_fold_paths = [
        p
        for p in TRAINING_DIR.glob("*.pkl.z")
        if p.name.split(".pkl.z")[0].split("_")[-1] != "full"
    ]
    ...
    best_score = None
    best_params = None
    for train_params, model_params in enumerate(
        tqdm(hyperparameter_grid.get_flattened_grid())
    ):
        res_pipe_dict = {}
        scores = []

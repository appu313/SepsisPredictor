...
def parse_and_clean_data(df: pd.DataFrame, id_col='patient_id', temporal_col='ICULOS', label_col='SepsisLabel', missing_values='mask') -> torch.tensor:
  """
  ...
  if missing_values == 'mask':
    ...
    mask = mask.drop(columns=[id_col, temporal_col, label_col]).add_suffix('_missing')
    df = df.fillna(0)
    ...
    raise ValueError("missing_values must be either 'mask', 'impute', or 'mask-impute'")
